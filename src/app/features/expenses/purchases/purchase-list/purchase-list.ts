...
export class PurchaseListComponent implements OnInit {
  ...
  private loadData(): void {
    ...
    this.purchaseService.getAll(this.pageIndex() + 1, this.pageSize()).subscribe({
      next: (res) => {
      ...
      error: () => {
        this.snackBar.open('Failed to load purchases.', 'Dismiss', { duration: 4000 });
        this.isLoading.set(false);
      },
    });
  }
  ...
  protected openAddDialog(): void {
    const ref = this.dialog.open(PurchaseFormComponent, {
      data: null,
      disableClose: true,
      maxWidth: '820px',
      width: '100%',
    ...
    ref.afterClosed().subscribe((result: PurchaseFormResult | undefined) => {
      if (result) {
        this.snackBar.open('Purchase recorded.', undefined, { duration: 3000 });
  ...
  protected navigateToView(item: Purchase): void {
    this.router.navigate(['/expenses/purchases', item.id]);
  }

  protected isActive(item: Purchase): boolean {
