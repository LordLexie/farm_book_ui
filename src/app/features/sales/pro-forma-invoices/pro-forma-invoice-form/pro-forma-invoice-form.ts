...
@Component({
  selector: 'app-pro-forma-invoice-form',
  imports: [
    DecimalPipe,
    ReactiveFormsModule,
    MatDialogModule,
  ...
  templateUrl: './pro-forma-invoice-form.html',
  styleUrl: './pro-forma-invoice-form.css',
})
export class ProFormaInvoiceFormComponent implements OnInit {
  ...
  private readonly dialogRef = inject(MatDialogRef<ProFormaInvoiceFormComponent>);
  protected readonly proFormaInvoice: ProFormaInvoice | null = inject(MAT_DIALOG_DATA);
  ...
  protected readonly isEditMode = this.proFormaInvoice !== null;

  protected readonly headerForm = this.fb.nonNullable.group({
    customer_id: [this.proFormaInvoice?.customer_id ?? ('' as unknown as number), [Validators.required]],
    status_id: [this.proFormaInvoice?.status_id ?? ('' as unknown as number)],
    currency_id: [this.proFormaInvoice?.currency_id ?? ('' as unknown as number), [Validators.required]],
    tax_id: [this.proFormaInvoice?.tax_id ?? (null as number | null)],
    date: [this.proFormaInvoice?.date?.substring(0, 10) ?? new Date().toISOString().substring(0, 10), [Validators.required]],
    discount: [this.proFormaInvoice?.discount ?? 0, [Validators.min(0), Validators.max(100)]],
  });
  ...
  private submitEdit(): void {
    if (this.headerForm.invalid || !this.proFormaInvoice) return;
    ...
    this.service.update(this.proFormaInvoice.id, payload).subscribe({
      next: () => this.dialogRef.close({ saved: true } satisfies ProFormaInvoiceFormResult),
      error: () => {
  ...
  private submitAdd(): void {
    ...
    const v = this.headerForm.getRawValue();
    const payload: StoreProFormaPayload = {
      customer_id: v.customer_id,
      currency_id: v.currency_id,
      tax_id: v.tax_id,
      date: v.date,
    ...
    this.service.create(payload).subscribe({
      next: () => this.dialogRef.close({ saved: true } satisfies ProFormaInvoiceFormResult),
      error: () => {
        this.errorMessage.set('Failed to create pro forma invoice. Please try again.');
        this.isSaving.set(false);
      },
    });
  }
